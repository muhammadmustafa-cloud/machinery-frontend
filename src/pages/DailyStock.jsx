...
export default function DailyStock() {
  ...
  const ins = sortedLedger.filter(e => e.type === 'IN');
  const outs = sortedLedger.filter(e => e.type === 'OUT');
  const rows = Math.max(ins.length, outs.length);
  const totalIn = ins.reduce((s, e) => s + e.quantity, 0);
  const totalOut = outs.reduce((s, e) => s + e.quantity, 0);
  ...
  const exportPDF = () => {
    ...
    // Total footer
    if (curY > H - 20) { doc.addPage(); drawPageHeader(); curY = 28; }
    ...
    doc.text(`+${totalIn}`, margin + colW - 2, curY + 5, { align: 'right' });
    doc.setTextColor(220, 38, 38);
    doc.text('TOTAL DEBIT', margin + colW + 2, curY + 5);
    doc.text(`-${totalOut}`, W - margin - 2, curY + 5, { align: 'right' });

    curY += 7;

    // Closing Balance
    doc.setFillColor(226, 232, 240); // slightly darker gray
    doc.rect(margin, curY, W - margin * 2, 8, 'F');
    doc.setDrawColor(203, 213, 225);
    doc.line(margin, curY + 8, W - margin, curY + 8);
    
    const netBalance = totalIn - totalOut;
    doc.setFontSize(8); doc.setFont('helvetica', 'bold');
    doc.setTextColor(50, 50, 50);
    doc.text('NET CLOSING BALANCE', margin + 2, curY + 5.5);
    
    if (netBalance >= 0) {
      doc.setTextColor(5, 150, 105);
      doc.text(`+${netBalance}`, W - margin - 2, curY + 5.5, { align: 'right' });
    } else {
      doc.setTextColor(220, 38, 38);
      doc.text(`${netBalance}`, W - margin - 2, curY + 5.5, { align: 'right' });
    }
  ...
  return (
    <div className="max-w-full mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">T-Account view — Credit (IN) and Debit (OUT) side by side.</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl overflow-hidden shadow-sm border border-gray-200 dark:border-zinc-800">
          {/* Split Header */}
          ...
          <div className="grid grid-cols-2 border-t-2 border-gray-300 dark:border-zinc-600">
            <div className="grid grid-cols-[1fr_90px] border-r border-gray-200 dark:border-zinc-700 bg-emerald-50 dark:bg-emerald-900/10">
              ...
              <div className="px-4 py-3 text-right text-sm font-bold text-emerald-700 dark:text-emerald-400">+{totalIn}</div>
            </div>
            <div className="grid grid-cols-[1fr_90px] bg-red-50 dark:bg-red-900/10">
              ...
              <div className="px-4 py-3 text-right text-sm font-bold text-red-700 dark:text-red-400">-{totalOut}</div>
            </div>
          </div>

          {/* Closing Balance */}
          <div className="border-t border-gray-300 dark:border-zinc-600 bg-gray-100 dark:bg-zinc-800/80 px-6 py-4 flex justify-between items-center">
            <span className="text-sm font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Net Closing Balance</span>
            <span className={`text-lg font-bold ${totalIn - totalOut >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
